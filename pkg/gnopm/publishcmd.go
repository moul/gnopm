...
func writeTxHandoff(e *Env, fs *flag.FlagSet, plans []plan, probe *Probe, key, out string) error {
	...
	// Data on stdout: the commands, so `gnopm publish -o tx.json | sh` still
	// works the way the script form does.
	e.printf("#!/bin/sh\n# generated by gnopm publish -o; review before running.\n")
	e.printf("# gnopm never signs: these are commands for you to run.\nset -e\n")
	for i, p := range paths {
		...
		e.printf("  -tx-path %s \\\n", shellQuote(p))
		e.printf("  -chainid %s \\\n", chain.ID)
		e.printf("  -account-number %s \\\n", num)
		e.printf("  -account-sequence %s \\\n", seq)
		e.printf("  %s\n", key)
		e.printf("%s broadcast -remote %s %s\n", gnokeyCmdOf(fs), chain.RPC, shellQuote(p))
	}
	if acctErr != nil {
		e.logf("\nnote     could not read %s from the chain (%v).\n", creator, acctErr)
		e.logf("         Fill in the account number and sequence yourself:\n")
